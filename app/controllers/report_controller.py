...
def get_main_dashboard_summary():
    try:
        now = datetime.now()
        first_day_this_month = now.replace(day=1, hour=0, minute=0,
        ...
        CALL {
            MATCH (u:User)
            WHERE u.createdAt >= $iso_this_month OR u.joinDate >= $iso_this_month
            RETURN count(u) AS new_users_this_month
        }
        CALL {
            MATCH (u:User)
            WHERE (u.createdAt >= $iso_last_month AND u.createdAt < $iso_this_month)
               OR (u.joinDate  >= $iso_last_month AND u.joinDate  < $iso_this_month)
        ...
        CALL {
            ...
            WHERE (k.isDeleted = false OR k.isDeleted IS NULL)
              AND k.createdAt >= $epoch_30_days_ago
            RETURN count(k) AS new_kawanss_30_days
        }

        CALL {
            MATCH (i:Infoss)
            WHERE (i.isDeleted = false OR i.isDeleted IS NULL)
              AND (i.uploadDate >= $iso_30_days_ago OR i.createdAt >= $iso_30_days_ago)
            RETURN count(i) AS new_infoss_30_days
        }

        RETURN total_users, total_infoss, total_kawanss,
               new_users_this_month, new_users_last_month,
               new_infoss_30_days, new_kawanss_30_days
        """
        ...
        try:
            ...
            if G.number_of_nodes() > 0:
                ...
                user_nodes = [n for n, a in G.nodes(data=True) if a.get('type') == 'user']

                def fmt(u):
                    return {
                        "id": u,
                        "name": G.nodes[u].get('name', u.replace('user_', '')),
                        "metrics": {
                ...
                top_10_centrality["degree"]      = [fmt(u) for u in sorted(user_nodes, key=lambda x: deg_cent.get(x, 0), reverse=True)[:10]]
                top_10_centrality["betweenness"] = [fmt(u) for u in sorted(user_nodes, key=lambda x: bet_cent.get(x, 0), reverse=True)[:10]]
                top_10_centrality["closeness"]   = [fmt(u) for u in sorted(user_nodes, key=lambda x: clo_cent.get(x, 0), reverse=True)[:10]]
                top_10_centrality["eigenvector"] = [fmt(u) for u in sorted(user_nodes, key=lambda x: eig_cent.get(x, 0), reverse=True)[:10]]
        ...
        return {
            ...
            "data": {
                ...
                "integrations": {
                    ...
                    "google_analytics": {"status": "connected", "active_users_now": 0}
                }
            }
        }
...
async def export_neo4j_to_csv():
    """CSV ringkasan user + jumlah post KawanSS."""
    query = """
    MATCH (u:User)
    OPTIONAL MATCH (u)-[:POSTED]->(k:KawanSS)
    WHERE k.isDeleted = false OR k.isDeleted IS NULL
    RETURN u.id AS ID, coalesce(u.nama, u.username, 'Unknown') AS Nama,
           count(k) AS Total_Post
    ...
    try:
        ...
        if not records:
            raise HTTPException(status_code=404,
                                detail="Tidak ada data di Neo4j untuk diexport.")
        ...
        return Response(
            ...
            headers={"Content-Disposition":
                     "attachment; filename=Laporan_SNA_Neo4j.csv"}
        )
    except Exception as e:
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500,
                            detail=f"Gagal export Neo4j ke CSV: {str(e)}")


async def export_instagram_to_csv():
    """CSV dari cache Instagram."""
    cache_file = "instagram_data_cache.json"
    if not os.path.exists(cache_file):
        raise HTTPException(
            status_code=404,
            detail="Cache Instagram tidak ditemukan. Jalankan /sna/ingest terlebih dahulu."
        )
    try:
        ...
        stream = io.StringIO()
        writer = csv.writer(stream)
        writer.writerow(["Post ID", "Timestamp", "Total Likes",
                         "Total Comments", "Caption"])
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={"Content-Disposition":
                     "attachment; filename=Laporan_SNA_Instagram.csv"}
        )
    except Exception as e:
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500,
                            detail=f"Gagal export Instagram ke CSV: {str(e)}")
...
def get_analytics_summary():
    return {
        ...
        "data": {"active_users": 150, "page_views": 1200}
    }