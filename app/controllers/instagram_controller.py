...
async def _check_config() -> None:
    ...
    if not config.IG_ACCESS_TOKEN or config.IG_ACCESS_TOKEN == "MASUKKAN_ACCESS_TOKEN_ANDA_YANG_SUDAH_DIGENERATE":
        raise HTTPException(
            ...
            detail="IG_ACCESS_TOKEN belum di-generate atau belum diatur di server. Silakan generate token di Meta Dashboard."
        )

async def _make_ig_api_request(endpoint: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Menghindari KeyError jika format error API berubah
        error_data = e.response.json()
        error_msg = error_data.get("error", {}).get("message", "Unknown error")
        raise HTTPException(
            status_code=e.response.status_code,
            detail=f"Error dari Instagram API: {error_msg}"
        )
...
async def get_user_profile() -> Dict[str, Any]:
    """Mengambil data profil akun Instagram."""
...
async def debug_token() -> Dict[str, Any]:
    ...
    return {
        ...
        "first_5_chars": token[:5] if token else "",
        "last_5_chars": token[-5:] if token else "",
        "loaded_token": token
    }